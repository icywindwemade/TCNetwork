...
template<typename ElementType,typename SetAllocator = NDefaultSetAllocator >
class TLookupMap;
 ...
 **/
template<typename KeyType,typename ValueType,UBOOL bInAllowDuplicateKeys,typename SetAllocator = NDefaultSetAllocator >
class TMapBase
{
public:
	...
	ValueType& Set(KeyInitType InKey,ValueInitType InValue)
	{
		...
		const NSetElementId PairId = Pairs.Add(NPairInitializer(InKey,InValue));
...
#pragma warning(disable : 4121) // alignment of a member was sensitive to packing
	...
	public:
		...
		NPairInitializer(KeyInitType InKey,ValueInitType InValue):
		Key(InKey),
		Value(InValue)
		{}
	};
	...
	public:
		...
		/** Initialization constructor. */
		FPair(const NPairInitializer& InInitializer)
		...
		operator NPairInitializer() const
		{
			return NPairInitializer(Key,Value);
		}
	};
...
#pragma warning(pop)
	...
	struct KeyFuncs : BaseKeyFuncs<FPair,KeyType,bInAllowDuplicateKeys>
	{
		...
		typedef const NPairInitializer&									ElementInitType;
		
		static KeyInitType GetSetKey(const NPairInitializer& Element)
		{
			return Element.Key;
		}
...
public:
	...
	{
		const NSetElementId PairId = Super::Pairs.FindId(Key);
		if(PairId.IsValidId())
		{
...
public:
	...
	{
		const NSetElementId PairId = Super::Pairs.Add(typename Super::NPairInitializer(InKey,InValue));
...
public:
	...
	INT AddItem(ElementInitType Element,UBOOL bAllowDuplicates=FALSE )
	{
		...
		else
		{
			...
			Super::Pairs.Add(typename Super::NPairInitializer(Element,Result));
		}
	 ...
	 */
	class NLookupMapElementReference
	{
	public:
		NLookupMapElementReference( TLookupMap& InMap, KeyType& InData, const INT InIndex )
		 ...
		 */
		NLookupMapElementReference& operator=( const KeyType& NewValue )
		{
	 ...
	 * This class is used to provide access to a map element in situations where the element data might change.
	 */
	class NLookupMapConstElementReference
	{
	public:
		NLookupMapConstElementReference( const TLookupMap& InMap, const KeyType& InData )
	 ...
	 */
	NLookupMapElementReference GetItem( INT ElementIndex )
	{
		//appCheckSlow(IsValidIndex(ElementIndex));
		return NLookupMapElementReference(*this, UniqueElements(ElementIndex), ElementIndex);
	}
	NLookupMapConstElementReference GetItem( INT ElementIndex ) const
	{
		//appCheckSlow(IsValidIndex(ElementIndex));
		return NLookupMapConstElementReference(*this, UniqueElements(ElementIndex));
	}
	NLookupMapElementReference operator()( INT ElementIndex )
	{
		//appCheckSlow(IsValidIndex(ElementIndex));
		return NLookupMapElementReference(*this, UniqueElements(ElementIndex), ElementIndex);
	}
	NLookupMapConstElementReference operator()( INT ElementIndex ) const
	{
		//appCheckSlow(IsValidIndex(ElementIndex));
		return NLookupMapConstElementReference(*this, UniqueElements(ElementIndex));
	}
