 ...
 * @file	TSparseArray.h
 *
 * Declares the sparse array class.
 */
#ifndef __TSPARSEARRAY_H__
#define __TSPARSEARRAY_H__
...
template<typename ElementType,typename Allocator = NDefaultSparseArrayAllocator >
class TSparseArray;

/**
 * The result of a sparse array allocation.
 */
struct NSparseArrayAllocationInfo
{
	INT Index;
	void* Pointer;
};
...
public:
	 ...
	 */
	NSparseArrayAllocationInfo Add()
	{
		NSparseArrayAllocationInfo Result;
	...
	{
		NSparseArrayAllocationInfo Allocation = Add();
	...
	void Sort()
	{
		if(Num() > 0)
		{
			...
			NElementOrFreeListLink* Element = &GetData(0);
//			INT NumOfElement = Num();
			Sort(Element, Num());
			
			//::Sort<NElementOrFreeListLink, ElementCompareClass<CompareClass>>(&GetData(0),Num());
		}
	}
	...
	{
		return	(Data.Num() + Data.GetSlack()) * sizeof(NElementOrFreeListLink) +
	...
	TSparseArray& operator=(const TSparseArray& InCopy)
	{
		if(this != &InCopy)
		{
			...
			if (TContainerTraits<ElementType>::NeedsConstructor)
			{
				...
				{
					const NElementOrFreeListLink& SourceElement = InCopy.GetData(Index);
					NElementOrFreeListLink& DestElement = GetData(Index);
			...
			else
			{
				...
				appMemcpy(Data.GetData(),InCopy.Data.GetData(),sizeof(NElementOrFreeListLink) * InCopy.GetMaxIndex());
			}
		}
		return *this;
	}
	...
	ElementType& operator()(INT Index)
	{
		...
		//appCheckSlow(Index < Data.Num());
		//appCheckSlow(Index < AllocationFlags.Num());
		//appCheckSlow(AllocationFlags(Index));
...
private:
	...
	public:
		...
		FORCEINLINE ItElementType* operator->() const { return &Array(GetIndex()); }
		FORCEINLINE const NRelativeBitReference& GetRelativeBitReference() const { return BitArrayIt; }
...
public:
	...
	template<typename SubsetAllocator = NDefaultBitArrayAllocator>
...
private:
	...
	{
	public:
		static INT Compare(const NElementOrFreeListLink& A,const NElementOrFreeListLink& B)
		{
			return CompareClass::Compare(*(ElementType*)&A.ElementData,*(ElementType*)&B.ElementData);
		}
	};
	
	TArray<NElementOrFreeListLink,typename Allocator::ElementAllocator> Data;
	...
	NElementOrFreeListLink& GetData(INT Index)
	{
		return ((NElementOrFreeListLink*)Data.GetData())[Index];
	}
	
	/** Accessor for the element or free list data. */
	const NElementOrFreeListLink& GetData(INT Index) const
	{
		return ((NElementOrFreeListLink*)Data.GetData())[Index];
	}
};
 ...
 */
inline void* operator new(size_t Size,const NSparseArrayAllocationInfo& Allocation)
{
