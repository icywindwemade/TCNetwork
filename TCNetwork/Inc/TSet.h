...
typename Allocator = NDefaultSetAllocator
>
class TSet;
...
public:
	...
	/** Comparison operator. */
	friend UBOOL operator==(const NSetElementId& A,const NSetElementId& B)
	{
...
public:
	...
	void Empty(INT ExpectedNumElements = 0)
	{
		...
		if(!ConditionalRehash(ExpectedNumElements,TRUE))
		{
			...
			if(HashSize)
			{
				...
				{
					GetTypedHash(HashIndex) = NSetElementId();
				}
			}
		}
	}
	...
	{
		return Elements.GetAllocatedSize() + (HashSize * sizeof(NSetElementId));
	}
	 ...
	 */
	UBOOL IsValidId(NSetElementId Id) const
	{
		return	Id.IsValidId() && 
		Id >= 0 &&
	...
	const ElementType& operator()(NSetElementId Id) const
	{
		//appCheckSlow(IsValidId(Id));
		return Elements(Id).Value;
	}
	
	/** Accesses the identified element's value. */
	ElementType& operator()(NSetElementId Id)
	{
		//appCheckSlow(IsValidId(Id));
		return Elements(Id).Value;
	}
	 ...
	 */
	NSetElementId Add(ElementInitType InElement,UBOOL* bIsAlreadyInSetPtr = NULL)
	{
		NSetElementId ElementId;
		...
		if(KeyFuncs::bAllowDuplicateKeys || !ElementId.IsValidId())
		{
			...
			NSparseArrayAllocationInfo ElementAllocation = Elements.Add();
			ElementId = NSetElementId(ElementAllocation.Index);
	 ...
	 */
	void Remove(NSetElementId ElementId)
	{
		if(HashSize)
		{
			...
			for(NSetElementId* NextElementId = &GetTypedHash(ElementBeingRemoved.HashIndex);
	 ...
	 */
	NSetElementId FindId(KeyInitType Key) const
	{
		if(HashSize)
		{
			for(NSetElementId ElementId = GetTypedHash(KeyFuncs::GetKeyHash(Key));
				ElementId.IsValidId();
	...
	{
		NSetElementId ElementId = FindId(Key);
		if(ElementId.IsValidId())
		{
	...
	INT RemoveKey(KeyInitType Key)
	{
		...
		if(HashSize)
		{
			NSetElementId* NextElementId = &GetTypedHash(KeyFuncs::GetKeyHash(Key));
	...
	UBOOL VerifyHashElementsKey(KeyInitType Key)
	{
		...
		if(HashSize)
		{
			...
			NSetElementId ElementId = GetTypedHash(KeyFuncs::GetKeyHash(Key));
...
private:
	...
	public:
		...
		mutable NSetElementId HashNextId;
	...
	mutable typename Allocator::HashAllocator::template ForElementType<NSetElementId> Hash;
	mutable INT HashSize;
	
	NSetElementId& GetTypedHash(INT HashIndex) const
	{
		return ((NSetElementId*)Hash.GetAllocation())[HashIndex & (HashSize - 1)];
	}
	
	/**
	 * Accesses an element in the set.
	 * This is needed because the iterator classes aren't friends of NSetElementId and so can't access the element index.
	 */
	const FElement& GetInternalElement(NSetElementId Id) const
	{
		return Elements(Id);
	}
	FElement& GetInternalElement(NSetElementId Id)
	{
		return Elements(Id);
	}
	 ...
	 * This is needed because the iterator classes aren't friends of NSetElementId and so can't access the NSetElementId private constructor.
	 */
	static NSetElementId IndexToId(INT Index)
	{
		return NSetElementId(Index);
	}
	
	/** Adds an element to the hash. */
	void HashElement(NSetElementId ElementId,const FElement& Element) const
	{
	...
	{
		// Free the old hash.
		Hash.ResizeAllocation(0,0,sizeof(NSetElementId));
		
		if(HashSize)
		{
			// Allocate the new hash.
			//appCheckSlow(!(HashSize&(HashSize-1)));
			Hash.ResizeAllocation(0,HashSize,sizeof(NSetElementId));
			for(INT HashIndex = 0;HashIndex < HashSize;HashIndex++)
			{
				GetTypedHash(HashIndex) = NSetElementId();
			}
			...
			{
				HashElement(NSetElementId(ElementIt.GetIndex()),*ElementIt);
			}
		}
	}
...
public:
	...
	public:
		...
		void RemoveCurrent()
		{
			...
			TBaseKeyIterator<false>::Id = NSetElementId();
		}
	private:
		TSet& Set;
	};
