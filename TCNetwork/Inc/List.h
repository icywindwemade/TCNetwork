...
class TLinkedList
{
	...
	public:
		...
		{
			return CurrentLink != NULL ? &PrivateBooleanHelper::Value : NULL;
		}
		bool operator !() const
		{
	 ...
	 * 
	 * This function is save to call even if the element is not linked.
	 */
	void Unlink()
	{
		if ( NextLink )
...
public:
	...
	/**
	 * Used to iterator over the elements of a linked list.
	 */
	class TIterator
	{
	private:
	...
	public:
		...
		operator PrivateBooleanType() const
		{
			return CurrentNode != NULL ? &PrivateBooleanHelper::Value : NULL;
		}
		bool operator !() const
		{
			return !operator PrivateBooleanType();
	 ...
	 * 
	 * @param	InElment	the value to add to the list.
	 * 						
	 ...
	 * @return	whether the node was successfully added into the list
	 */
	UBOOL InsertNode( const ElementType& InElement, TDoubleLinkedListNode* NodeToInsertBefore )
	{
	...
	void Clear()
	{
		TDoubleLinkedListNode* pNode = NULL;
		while ( HeadNode != NULL )
		{
...
protected:
	/**
	 * Updates the size reported by Num(). Child classes can use this function to conviently
	 * hook into list additions/removals.
	 * 
	 * @param	NewListSize		the new size of this list
	 */
...
private:
	TDoubleLinkedList* HeadNode;
	TDoubleLinkedList* TailNode;
	INT ListSize;
};
